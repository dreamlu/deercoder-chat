...
type Message struct {
	...
	CreateTime  time2.CTime `json:"create_time"`  //创建时间
}
...
func DistributeGroup(uids string) (groupId string, err error) {
	...
	groupUserSql := `select (count(*) > 1) as num 
					...
					group by a.group_id
					having num = 1`
	err = gt.DBTooler().ValidateSQL(groupUserSql)
	if err == nil {
...
func CreateGroupMsg(msg *proto.Message) (err error) {
	...
	dba := gt.DBTooler().DB.Exec(sql, msg.Uuid, msg.GroupId, msg.Content, msg.FromUid, msg.ContentType)
...
func CreateGroupLastMsg(group_id string, uid int64, last_group_msg_uuid string) (err error) {
	...
	dba := gt.DBTooler().DB.Exec(sql, group_id, uid, last_group_msg_uuid)
...
func GetAllGroupMsg(group_id string) ([]*proto.Message, error) {
	...
	//拉取该群聊的所有消息
	sql := fmt.Sprintf("select %s from group_msg where group_id=?", gt.GetColSQL(GroupMsg{}))
	var msg []*proto.Message

	gt.DBTooler().DB.Raw(sql, group_id).Scan(&msg)
	...
	for k, v := range msg { //查询对应的头像,用户名等信息
		...
		gt.DBTooler().DB.Raw(sql, v.FromUid).Scan(&msg[k])
	}
...
func GetGroupLastMsg(group_id string, uid int64) ([]*proto.Message, error) {
	...
	gt.DBTooler().DB.Raw(sql2, group_id, uid).Scan(&value)

	if value.Value == "" {
		return nil, errors.New("暂无离线消息")
	}
	//2.拉取离线后的该群聊的所有消息
	sql := fmt.Sprintf("select %s from group_msg where group_id=? and id >= (select id from group_msg where uuid = ?)", gt.GetColSQL(GroupMsg{}))

	var msg []*proto.Message

	gt.DBTooler().DB.Raw(sql, group_id, value.Value).Scan(&msg)
...
// 已读消息
func ReadGroupLastMsg(group_id string, uid int64) (result.MapData, error) {

	var info result.MapData
	sql2 := "update `group_last_msg` set is_read=1 where is_read=0 and group_id=? and uid=?"
	dba := gt.DBTooler().DB.Exec(sql2, group_id, uid)
	num := dba.RowsAffected
	if dba.Error != nil {
		return info, dba.Error
	} else if num == 0 && dba.Error == nil {
		return result.MapNoResult, errors.New(result.MsgNoResult)
	} else {
		return result.MapUpdate, nil
	}
}
...
func MassMessage(group_ids, send_uids, from_uid, content string) interface{} {
	...
	if group_ids == "" {
		return result.GetMapData(272, "group_ids不能为空")
	}
	...
	gt.DBTooler().DB.Exec(sql)
	dba := gt.DBTooler().DB.Exec(sql2) //创建存储群聊消息

	if dba.Error != nil {
		return result.GetError(dba.Error.Error())
	}

	return result.MapCreate
}
...
func GetUserList(uid int64) (users []*proto.ChatUser, err error) {
	...
	sql := `select a.id,name,headimg,introduce,createtime,b.group_id 
			...
			and a.id != ?`
	dba := gt.DBTooler().DB.Raw(sql, uid, uid).Scan(&users)
...
func GetGroupUser(group_id string, gusers []*proto.GroupUser) error {
	...
	dba := gt.DBTooler().DB.Raw("select id,group_id,uid from `group_users` where group_id = ?", group_id).Scan(&gusers)
	return dba.Error
}
