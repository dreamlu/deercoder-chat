...
func SetRouter() *gin.Engine {
	...
	v1 := router.Group("/api/v1")
	{
		...
		{
			chats.GET("/", main.Chat)
			chats.GET("/ws", main.ChatWS)
			chats.GET("/getglmsg", main.GetGroupLastMsg)
			chats.POST("/massmsg", main.MassMessage)
			chats.POST("/readmsg", main.ReadGroupLastMsg)
			chats.POST("/distchat", main.DistributeGroup)
		}
		users := v.Group("/user")
		{
			users.POST("/create", main.Create)
			users.PUT("/update", main.Update)
			users.DELETE("/delete", main.DeleteById)
			users.GET("/getbysearch", main.GetBySearch)
			users.GET("/getbyid", main.GetById)
		}
	}
	//不存在路由
